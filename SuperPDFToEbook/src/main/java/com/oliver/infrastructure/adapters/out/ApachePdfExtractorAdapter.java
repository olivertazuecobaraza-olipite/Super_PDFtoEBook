...
public class ApachePdfExtractorAdapter implements PdfExtractorPort {
    ...
    @Override
    public EbookPagesMap extractPages(File pdfFile, String userTextIndex,
        ...
        try {
            // Usamos MemoryUsageSetting.setupTempFileOnly() para que los PDFs gigantes
            // (1000+ páginas) NO colapsen la Memoria RAM.
            try (PDDocument document = PDDocument.load(pdfFile,
                ...
                if (userTextIndex != null && !userTextIndex.trim().isEmpty()) {
                    ...
                    if (outline != null && outline.getFirstChild() != null) {
                        ...
                        System.out
                                .println("⚠️ El PDF no tiene marcadores. Generando índice de paginación de respaldo.");
                ...
                for (int i = 0; i < totalPages; i++) {
                    ...
                    if (Thread.currentThread().isInterrupted()) {
                        throw new InterruptedException(
                                "El usuario o el sistema cerró la aplicación durante el procesado.");
                    }
                    int pageNum = i + 1;
    ...
    private String buildFallbackOutline(int totalPages) {
        ...
        for (int i = step; i < totalPages; i += step) {
            sb.append("<li><a href=\"#\" onclick=\"window.goToPage(").append(i).append("); return false;\">Página ")
                    .append(i).append("</a></li>");
        }

        if (totalPages > 1) {
            sb.append("<li><a href=\"#\" onclick=\"window.goToPage(").append(totalPages)
                    .append("); return false;\">◼ Final (Página ").append(totalPages).append(")</a></li>");
        }
        sb.append("</ul>");
        return sb.toString();
    }

    // Navegación recursiva del DOM de los marcadores del PDF original (La Jerarquía
    // #2)
